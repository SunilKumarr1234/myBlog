...
const PicoCTF = () => {
  ...
  const downloadPDF = (category) => {
    ...
    const pdfPath = `/pdfs/${fileName}` // ✅ Corrected Path
      ...
      <div className='font-mono text-white px-6 sm:px-14 mt-10 pb-32'>
        <ul className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-10 gap-x-6'>
          {[
            ...
            <li key={index} className='bg-gray-700 bg-opacity-10 rounded-lg p-4 relative'>
              <button
                ...
                {category.name === 'Cryptography' ? (
                  <>
                    <li className='hover:text-green-500'>
                      <Link to='https://play.picoctf.org/practice/challenge/351?category=2&page=1' target='_blank'>
                        ➤ HideToSee
                      </Link>
                    </li>
                    <li className='hover:text-green-500'>
                      <Link to='https://play.picoctf.org/practice/challenge/418?category=2&page=1' target='_blank'>
                        ➤ interencdec
                      </Link>
                    </li>
                  </>
